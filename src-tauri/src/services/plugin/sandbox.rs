...
impl WasmSandbox {
    ...
    pub fn validate_module(&self, wasm_bytes: &[u8]) -> Result<(), CntrlError> {
        Module::validate(&self.engine, wasm_bytes)
            .map_err(|e| CntrlError::Plugin(format!("Invalid WASM module: {}", e)))
    }
    ...
    pub async fn run_plugin_zip(&self, zip_path: &Path) -> Result<String, CntrlError> {
        ...
        let file = File::open(zip_path)
            .map_err(|e| CntrlError::Plugin(format!("Could not open plugin zip: {}", e)))?;
        let mut archive = zip::ZipArchive::new(file)
            .map_err(|e| CntrlError::Plugin(format!("Invalid plugin zip: {}", e)))?;

        // 2. Read manifest.json
        let manifest: PluginManifest = {
            let mut manifest_file = archive
                .by_name("manifest.json")
                .map_err(|_| CntrlError::Plugin("Missing manifest.json in plugin".to_string()))?;
            let mut manifest_str = String::new();
            manifest_file
                .read_to_string(&mut manifest_str)
                .map_err(|e| CntrlError::Plugin(format!("Could not read manifest.json: {}", e)))?;
            serde_json::from_str(&manifest_str)
                .map_err(|e| CntrlError::Plugin(format!("Invalid manifest.json: {}", e)))?
        };

        // 3. Read the entrypoint wasm module
        let mut wasm_file = archive.by_name(&manifest.entrypoint).map_err(|_| {
            CntrlError::Plugin(format!(
                "Missing entrypoint {} in plugin",
                manifest.entrypoint
            ))
        })?;
        ...
        wasm_file
            ...
            .map_err(|e| CntrlError::Plugin(format!("Could not read wasm: {}", e)))?;
            ...
            .contains(&PluginPermission::NetworkAccess)
        {
            linker
                .func_wrap(
                    "cntrl",
                    "fetch",
                    |_caller: Caller<'_, ()>, _ptr: i32, _len: i32| -> i32 {
        ...
        if manifest
            .permissions
            .contains(&PluginPermission::FileSystemAccess)
        {
            linker
                .func_wrap(
                    "cntrl",
        ...
        if manifest
            .permissions
            .contains(&PluginPermission::IntentExecution)
        {
            linker
                .func_wrap(
                    "cntrl",
        ...
        Ok(format!(
            "Plugin {} v{} executed successfully",
